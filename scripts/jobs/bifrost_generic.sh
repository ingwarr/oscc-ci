...
function main {
  ...
  local scp_opts='-oUserKnownHostsFile=/dev/null -oStrictHostKeyChecking=no'
    if [ -z ${hw_enabled} ] 
	 ...
	 sshpass -p 'r00tme' scp -r ${scp_opts} $HELPERS_DIR/vm/ps_bm.xml root@${env_ip}:/etc/libvirt/qemu/
	 execute_ssh_cmd ${env_ip} root r00tme "qemu-img create -f qcow2 /var/lib/libvirt/images/ps_bm.qcow2 15G; num=0; virsh define /etc/libvirt/qemu/ps_bm.xml && for ((num=1; num<=3; num++)); do virt-clone -o ps_bm -n ps_bm-${num} --auto-clone; done "
  fi
